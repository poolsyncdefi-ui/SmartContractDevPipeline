...
def share_to_github_gists(config):
    """
    Partage le projet sur GitHub Gists en gérant la limite de taille (50 Mo).
    Lit la config, filtre les fichiers, génère le contenu formaté et l'upload.
    """
    print("\n" + "="*60)
    print("SHARE TO GITHUB GISTS (GESTION INTELLIGENTE DE LA TAILLE)")
    print("="*60)

    # 1. Récupération et validation du token GitHub
    github_token = config.get("GITHUB_TOKEN")
    if not github_token or github_token.strip() == "":
        print("\n🔑 TOKEN GITHUB REQUIS")
        print("Pour créer un token:")
        ...
        print("2. 'Generate new token (classic)'")
        print("3. Donnez un nom")
        print("4. Sélectionnez 'gist' (ET 'repo' si vous voulez aussi pusher le repo)")
        print("5. 'Generate token'")
        print("6. Copiez le token (commence par 'ghp_')\n")
        github_token = input("Collez votre token GitHub: ").strip()
        if github_token:
            ...
            print("✗ Aucun token fourni, opération annulée.")
            return False

    # 2. Récupération du chemin du projet et validation
    project_path = Path(config.get("PROJECT_PATH", "."))
    if not project_path.exists():
        print(f"✗ Chemin du projet introuvable: {project_path}")
        return False
    print(f"✓ Chemin du projet: {project_path}")

    # 3. Collecte et filtrage des fichiers
    ...
    if not all_files:
        print("✗ Aucun fichier valide trouvé après filtrage.")
        return False

    print(f"✓ {len(all_files)} fichiers valides trouvés.")

    # 4. Construction incrémentale des parties de Gist
    print("\n✍️ Construction du contenu des Gists...")
    gist_parts = []
    current_part_content = []
    current_part_size = 0
    part_number = 1
    max_gist_size_bytes = 50 * 1024 * 1024  # 50 Mo, limite stricte de l'API
    # Limite de sécurité légèrement inférieure pour éviter les dépassements dus aux en-têtes
    safe_max_size = int(max_gist_size_bytes * 0.95)  # ~47.5 Mo

    # En-tête commun pour chaque partie
    def get_part_header(part_num, total_files):
        header = [
            f"PROJECT: {config.get('PROJECT_NAME')}\n",
            f"DATE: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n",
            f"PART: {part_num}\n",
            f"FILES IN THIS PART: {total_files}\n",
            "="*80 + "\n\n"
        ]
        return "".join(header)

    # Fonction pour finaliser une partie
    def finalize_part(part_content_list, part_num):
        if not part_content_list:
            return None
        # Ajouter l'en-tête au début du contenu de la partie
        header = get_part_header(part_num, len([f for f in part_content_list if f.startswith("FICHIER :")]))
        return header + "".join(part_content_list)

    for file_idx, file_path in enumerate(all_files, 1):
        # Construction de l'en-tête du fichier
        file_header = f"FICHIER : {file_path.absolute()}\n"
        file_content_lines = []
        ...
        try:
            ...
            # Formater chaque ligne avec son numéro
            for line_num, line in enumerate(lines, 1):
                file_content_lines.append(f"{line_num:4d}: {line.rstrip()}\n")

        except Exception as e:
            file_content_lines = [f"[ERREUR DE LECTURE: {e}]\n"]

        # Calculer la taille potentielle de ce fichier dans la partie courante
        potential_part_content = current_part_content + [file_header] + file_content_lines
        # Si ce n'est pas le premier fichier, on ajoute un saut de ligne avant le prochain
        if current_part_content:
            potential_part_content = current_part_content + ["\n"] + [file_header] + file_content_lines
        else:
            potential_part_content = current_part_content + [file_header] + file_content_lines

        potential_part_str = "".join(potential_part_content)
        potential_part_size = len(potential_part_str.encode('utf-8'))

        # Vérifier si l'ajout du fichier dépasse la limite de sécurité
        if potential_part_size > safe_max_size and current_part_content:
            # Finaliser la partie courante
            finalized_part = finalize_part(current_part_content, part_number)
            if finalized_part:
                gist_parts.append({
                    "number": part_number,
                    "content": finalized_part,
                    "size": len(finalized_part.encode('utf-8'))
                })
                print(f"  ✓ Partie {part_number} finalisée avec {len([f for f in current_part_content if f.startswith('FICHIER :')])} fichiers.")
            # Réinitialiser pour la nouvelle partie
            part_number += 1
            current_part_content = [file_header] + file_content_lines
        else:
            # Ajouter le fichier à la partie courante
            if current_part_content:
                current_part_content.append("\n")  # Saut de ligne entre les fichiers
            current_part_content.append(file_header)
            current_part_content.extend(file_content_lines)
    ...
    print(f"\n📊 Total: {len(gist_parts)} partie(s) de Gist à créer.")

    # 5. Upload des parties sur GitHub Gists
    print("\n🚀 Upload vers GitHub Gists...")
    headers = {
    ...
    for part in gist_parts:
        print(f"\n  Envoi de la partie {part['number']} ({part['size'] / (1024*1024):.2f} Mo)...")
        data = {
            "description": f"{config.get('PROJECT_NAME')} - Partie {part['number']}/{len(gist_parts)} - {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}",
            "public": True,  # Vous pouvez rendre ceci configurable si nécessaire
            "files": {
                f"PROJECT_PART{part['number']}.txt": {
        ...
        try:
            response = requests.post("https://api.github.com/gists", headers=headers, json=data, timeout=60)
            if response.status_code == 201:
                ...
                print(f"    ✅ Créé: {gist_data['html_url']}")
            else:
                print(f"    ❌ Erreur HTTP {response.status_code}: {response.text[:200]}")
                # On continue pour essayer de poster les autres parties
        except requests.exceptions.RequestException as e:
            print(f"    ❌ Erreur réseau ou de requête: {e}")

    # 6. Création du fichier d'index local
    if uploaded_gists:
        index_file = Path("GISTS_INDEX.txt")
        timestamp = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        with open(index_file, "w", encoding="utf-8") as f:
            f.write("="*80 + "\n")
            f.write(f"INDEX DES GISTS GITHUB - {config.get('PROJECT_NAME')}\n")
            f.write("="*80 + "\n")
            f.write(f"Date de création: {timestamp}\n")
            f.write(f"Projet: {config.get('PROJECT_NAME')}\n")
            f.write(f"Chemin source: {project_path}\n")
            f.write(f"Total parties: {len(gist_parts)}\n")
            f.write(f"Parties uploadées avec succès: {len(uploaded_gists)}\n")
            f.write(f"Visibilité: Public\n")
            f.write("-"*80 + "\n\n")
            f.write("LIENS VERS LES GISTS:\n\n")
            for gist in sorted(uploaded_gists, key=lambda x: x['part']):
                f.write(f"Partie {gist['part']}: {gist['url']}\n")
            f.write("\n" + "="*80 + "\n")
        print(f"\n📄 Fichier d'index créé: {index_file.absolute()}")

        print("\n" + "="*60)
        print("✅ OPÉRATION TERMINÉE")
        print("="*60)
        print(f"Gists créés avec succès: {len(uploaded_gists)}/{len(gist_parts)}")
        return True
    else:
...
def split_and_upload_from_content(config, github_token, content, is_public=True):
    ...
    max_lines_per_gist = 7000  # Environ 400-500KB
