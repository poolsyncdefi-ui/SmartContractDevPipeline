...
class LearningAgent(BaseAgent):
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            learning_config = self._agent_config.get("learning", {})
            if learning_config.get("auto_train", True):
        ...
        for dep in dependencies:
            try:
                if dep == "tester":
                    ...
                    from agents.formal_verification.agent import FormalVerificationAgent
                elif dep == "monitoring":
    ...
    async def _init_test_optimizer(self) -> Dict[str, Any]:
        """Initialise l'optimiseur de tests (extrait de votre code)"""
        models_config = self._agent_config.get("models", {})
    ...
    async def _init_anomaly_detector(self) -> Dict[str, Any]:
        """Initialise le détecteur d'anomalies (extrait de votre code)"""
        models_config = self._agent_config.get("models", {})
    ...
    async def shutdown(self) -> bool:
        ...
        try:
            ...
            stats_file = reports_path / "learning_stats.json"
        ...
        try:
            ...
            if msg_type == "predict_gas":
                ...
                return Message(
                    ...
                    content=result,
                    message_type="gas_prediction_result",
                ...
                return Message(
                    ...
                    recipient=message.sender,
                    content=result,
                    message_type="vulnerability_classification",
                    correlation_id=message.message_id
                )
                ...
                return Message(
                    ...
                    recipient=message.sender,
                    content=result,
                    message_type="test_optimization",
                    correlation_id=message.message_id
                )
                ...
                return Message(
                    ...
                    recipient=message.sender,
                    content=result,
                    message_type="anomaly_detection",
                    correlation_id=message.message_id
                )
                ...
                return Message(
                    ...
                    message_type="insights_list",
                    correlation_id=message.message_id
                )
            
            elif msg_type == "add_training_example":
                task_type = LearningTaskType(message.content.get("task_type"))
                ...
                return Message(
                    ...
                    message_type="training_example_added",
                    correlation_id=message.message_id
                )
    ...
    async def _continuous_learning(self):
        ...
        while self._status == AgentStatus.READY:
            try:
                ...
                # Entraîner l'optimiseur de tests (extrait de votre code)
                ...
                # Entraîner le détecteur d'anomalies (extrait de votre code)
    ...
    # === MÉTHODES DE VOS EXTRAITS ===

    async def _train_test_optimizer(self):
        """Entraîne l'optimiseur de stratégies de test (extrait de votre code)"""
        if not SKLEARN_AVAILABLE:
            return
    ...
    async def _train_anomaly_detector(self):
        """Entraîne le détecteur d'anomalies (extrait de votre code)"""
        if not SKLEARN_AVAILABLE:
            return
    ...
    async def _save_model(self, name: str, model_data: Dict):
        ...
        try:
            ...
            file_path = model_path / f"{name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pkl"
    ...
    def _extract_anomaly_features(self, data: Dict[str, Any]) -> Optional[List[float]]:
        """Extrait les features pour la détection d'anomalies (extrait de votre code)"""
        try:
            return [
    ...
    def _extract_vulnerability_features_from_code(self, contract_code: str) -> Optional[List[float]]:
        """Extrait les features du code pour la classification (extrait de votre code)"""
        try:
            return [
    ...
    async def classify_vulnerability(self, contract_code: str) -> Dict[str, Any]:
        """Classifie les vulnérabilités potentielles d'un contrat (extrait de votre code)"""
    ...
    async def optimize_test_strategy(self, contract_type: str, history: List[Dict]) -> Dict[str, Any]:
        """Optimise la stratégie de test (extrait de votre code)"""
    ...
    async def detect_anomalies(self, metrics: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Détecte les anomalies dans les métriques (extrait de votre code)"""
    ...
    async def get_vulnerability_stats(self) -> Dict[str, Any]:
        ...
        # Cette méthode pourrait analyser les données d'entraînement
        # Pour l'instant, retourne des stats simulées
        return stats
