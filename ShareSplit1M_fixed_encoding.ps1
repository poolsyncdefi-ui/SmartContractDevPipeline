...
[filter "remove-secrets"]
    smudge = cat
    clean = sed -E ''s/REPLACE_PATTERNS/***REMOVED***/g''
"@
    
    # Remplacer le placeholder par les patterns réels
    $patterns = $tokenPatterns -join ''|''
    $filterConfigContent = $filterConfigContent -replace ''REPLACE_PATTERNS'', $patterns
    
    $filterConfigContent | Out-File "git-filter-config.txt" -Encoding UTF8
